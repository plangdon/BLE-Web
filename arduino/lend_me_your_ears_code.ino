...
Relay relay3(3, RELAY3);

// timing parameters
long currentMillis, previousMillisRelay1, previousMillisRelay2, previousMillisRelay3;
...
void setup() 
{
 ...
 /** 
  ...
  blePeripheral.setLocalName("bob");
...
void loop() 
{
  ...
  if (relay3.getIntervalState())
  {  
    if ((currentMillis - previousMillisRelay3) > relay3.getInterval())
    {
      relay3.toggle();
...
void relayCharacteristicWritten(BLECentral& central, BLECharacteristic& characteristic)
{
  ...
  switch(relayData[0])
  {
    ...
    case 3:
      // relay 3 selected
      Serial.print("relay3: ");Serial.print(relayData[0]);Serial.print(" interval: ");Serial.println(interval);
      if (interval != 0) 
      {
        relay3.setIntervalState(true);
        relay3.setRelayState(true);
        relay3.setInterval(interval);
      } else {
        if (relay3.getRelayState()) 
        { 
          relay3.setRelayState(false); 
          relay3.turnOff(); 
          relay3.setIntervalState(false);
          relay3.setInterval(0); 
        }
        else {
          relay3.setRelayState(true); 
          relay3.turnOn();
